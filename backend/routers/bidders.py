...
def process_bidder_documents(
    tender_id: str, bidder_id: str, file_paths: list[str], file_names: list[str]
):
    try:
        db = get_db()
        manager.broadcast(
        ...
        if not criteria:
            logger.error(
                f"No approved criteria found for tender {tender_id}. Cannot process bidder {bidder_id}."
            )
        ...
        for fpath, fname in zip(file_paths, file_names, strict=False):
            ...
            try:
                ...
                security_report["scanned_files"].append({"filename": fname, "report": scan_res})
                ...
                manager.broadcast(
                    bidder_id,
                    {
                        ...
                        "status": (
                            "failed"
                            if scan_res["injection_detected"]
                            else ("warning" if scan_res["tampering_detected"] else "passed")
                        ),
                        "report": scan_res,
                    },
                )
                ...
                if scan_res["injection_detected"]:
                    logger.critical(
                        f"SecurityShield: CRITICAL prompt injection detected in {fname}! Halting pipeline."
                    )
                    try:
                        db.table("bidders").update(
                            {
                        ...
                        logger.warning(
                            f"Failed to update security_report column (schema cache stale?): {db_err}"
                        )
                        db.table("bidders").update(
                            {
                                "status": "failed",
        ...
        manager.broadcast(bidder_id, {"type": "documents_extracted", "documents": all_docs_text})

        # 3. For each criterion, run DocProbe
        total_criteria = len(criteria)
        db.table("bidders").update({"total_count": total_criteria, "processed_count": 0}).eq(
            "id", bidder_id
        ).execute()
        ...
        for i, raw_criterion in enumerate(criteria):
            ...
            try:
                ...
                db.table("bidders").update({"current_step": f"Analyzing {current_label}..."}).eq(
                    "id", bidder_id
                ).execute()

                # Broadcast criterion start event
                manager.broadcast(bidder_id, {"type": "criterion_start", "criterion": criterion})
                ...
                for doc in all_docs_text:
                    is_relevant = any(
                        k in doc["filename"].lower() or k in doc["text"].lower()[:500]
                ...
                def handle_token(token: str, cid: str = str(criterion["id"])):
                    manager.broadcast(
                        bidder_id,
                        {"type": "llm_token", "criterion_id": cid, "token": token},
                    )

                extraction = extract_value_for_criterion(
                    dict(criterion), final_context, on_token=handle_token
                )
                ...
                try:
                        ...
                        .insert(
                            {
                                ...
                                "extraction_confidence": extraction.alignment_score * 0.5
                                + extraction.authenticity_score * 0.5,
                            }
                        )
                        .execute()
                    )
                    ...
                    if ext_res.data and len(ext_res.data) > 0:
                        ...
                        if source_doc_name:
                            for sf in security_report.get("scanned_files", []):
                                if sf["filename"] == source_doc_name:
                                    is_tampered_source = sf["report"].get(
                                        "tampering_detected", False
                                    )
                                    break
                        ...
                        verdict = compute_verdict(
                            dict(criterion),
                            dict(extraction_dict),
                            is_tampered_source=is_tampered_source,
                        )
                ...
                logger.error(
                    f"Error extracting for criterion {criterion['id']}: {e}", exc_info=True
                )
            finally:
                # IMPORTANT: Always increment processed_count so the UI doesn't hang
                try:
                    db.table("bidders").update({"processed_count": i + 1}).eq(
                        "id", bidder_id
                    ).execute()
        ...
        try:
            ...
            db.table("bidders").update(
                {"status": "complete", "current_step": "Processing finished"}
            ).eq("id", bidder_id).execute()

        # Broadcast completion
        manager.broadcast(
            bidder_id,
            {"type": "status_update", "status": "complete", "current_step": "Processing finished"},
        )
        ...
        try:
            get_db().table("bidders").update(
                {"status": "failed", "current_step": f"Error: {str(global_e)}"}
            ).eq("id", bidder_id).execute()
            manager.broadcast(
                bidder_id,
                {
...
async def upload_bidder_documents(
    ...
    background_tasks.add_task(
        process_bidder_documents, tender_id, bidder_id, saved_paths, file_names
    )
