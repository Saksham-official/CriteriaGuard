...
async def export_tender_report(tender_id: str, officer_id: str = "SYSTEM_OR_OFFICER"):
    try:
        db = get_db()
        # Fetch tender
        t_res = db.table("tenders").select("*").eq("id", tender_id).execute()
        if not t_res.data:
            raise HTTPException(status_code=404, detail="Tender not found")
        tender: dict[str, Any] = t_res.data[0]  # type: ignore[assignment]
        ...
        for raw_bidder in bidders:
            ...
            extractions: list[dict[str, Any]] = [dict(e) for e in ext_res.data]  # type: ignore[arg-type]
            verdicts: list[dict[str, Any]] = [dict(v) for v in verdict_res.data]  # type: ignore[arg-type]
